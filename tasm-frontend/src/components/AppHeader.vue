<template>
  <header
    class="fixed top-0 left-0 w-full h-[60px] z-50 bg-[#FAFAF8] dark:bg-stone-950 border-b border-[#EEEBE4] dark:border-stone-800 shadow-sm flex justify-between items-center px-8 font-['Inter'] antialiased tracking-tight"
  >
    ...
    <div class="flex-1 flex justify-start pl-12">
      <div class="relative w-96">
        ...
        <InputText
          v-model="query"
          class="w-full !bg-surface !border-border-default !rounded-full !py-1.5 !pl-10 !pr-4 !text-body !font-body"
          placeholder="Search routes..."
          type="text"
        />
        <div
          v-if="filteredRoutes.length > 0"
          class="absolute left-0 right-0 top-[calc(100%+8px)] rounded-xl border border-border-default bg-white shadow-lg overflow-hidden"
        >
          <RouterLink
            v-for="item in filteredRoutes"
            :key="item.name"
            :to="item.to"
            class="flex items-center gap-3 px-4 py-3 text-sm text-stone-700 hover:bg-amber-50"
          >
    ...
    <div class="flex items-center gap-inline">
      ...
      >
        <img
          ...
          data-alt="Professional headshot of an administrator"
