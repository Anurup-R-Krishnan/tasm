<template>
  <div
    class="qr-scanner-container relative overflow-hidden rounded-2xl bg-black aspect-square max-w-sm mx-auto shadow-2xl"
  >
    <div id="qr-reader" class="w-full h-full"></div>

    <!-- Scanning Overlay -->
    <div
      class="absolute inset-0 pointer-events-none border-2 border-primary/50 m-12 rounded-xl animate-pulse"
    >
      ...
      <!-- Scanning Line -->
      <div
        class="absolute top-0 left-0 right-0 h-1 bg-primary/80 shadow-[0_0_15px_rgba(113,44,0,0.8)] animate-scan"
      ></div>
    </div>

    <div
      v-if="error"
      class="absolute inset-0 bg-black/80 flex items-center justify-center p-6 text-center"
    >
      <div class="text-white">
        <span class="material-symbols-outlined text-error text-4xl mb-2">error</span>
        <p class="font-body text-sm">{{ error }}</p>
        <button @click="retry" class="mt-4 px-4 py-2 bg-primary text-white rounded-lg text-sm">
          Retry
        </button>
      </div>
    </div>
