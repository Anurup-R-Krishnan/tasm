  ...
  <main class="p-page-margin max-w-[1400px] mx-auto space-y-section-gap">
    ...
    <div class="bg-surface rounded-xl border border-border-default shadow-[0_4px_16px_rgba(0,0,0,0.02)] overflow-hidden">
      <div class="p-4 border-b border-border-default flex justify-between items-center bg-surface-subtle">
        <div class="flex gap-inline">
          <div class="relative w-64">
            ...
            <InputText v-model="searchQuery" class="w-full !bg-surface !border-border-default !rounded-lg !py-1.5 !pl-9 !pr-3 !text-body !font-body" placeholder="Filter by item name..." />
          </div>
          <Select v-model="selectedCategory" :options="categoryOptions" class="min-w-56" option-label="label" option-value="value" placeholder="All Categories" />
        </div>
        <div class="flex gap-2">
          <Button aria-label="Filter list" icon="pi pi-filter" rounded severity="secondary" text />
          <Button aria-label="Toggle columns" icon="pi pi-table" rounded severity="secondary" text />
        </div>
      </div>
      ...
      <div class="bg-surface rounded-xl shadow-sm overflow-hidden border-t border-border-default">
        ...
        <DataTable :value="filteredConsumables" :loading="loading" paginator :rows="10" tableStyle="min-width: 50rem" class="w-full" dataKey="id">
          <template #empty>
            No consumables matched the current filters.
          </template>
...
const metrics = computed(() => {
  const lowStockItems = consumables.value.filter((item) => item.currentStock <= item.reorderLevel).length
