<template>
  <main class="space-y-section-gap pb-24">
    ...
    <!-- Step Indicator -->
    <div
      class="w-full bg-surface border border-border-default rounded-xl p-6 shadow-sm flex items-center justify-between"
    >
      <!-- Step 1 -->
      <div class="flex flex-col items-center gap-2 relative z-10 flex-1">
        <div
          class="w-8 h-8 rounded-full bg-status-in-stock text-white flex items-center justify-center font-medium text-sm"
        >
      ...
      <!-- Connector -->
      <div class="h-px bg-status-in-stock flex-1 -mx-8"></div>
      <!-- Step 2 -->
      <div class="flex flex-col items-center gap-2 relative z-10 flex-1">
        <div
          class="w-8 h-8 rounded-full bg-status-in-stock text-white flex items-center justify-center font-medium text-sm"
      ...
      <!-- Connector -->
      <div class="h-px bg-amber-600 flex-1 -mx-8"></div>
      <!-- Step 3 (Active) -->
      <div class="flex flex-col items-center gap-2 relative z-10 flex-1">
        <div
          class="w-8 h-8 rounded-full bg-amber-600 text-white flex items-center justify-center font-medium text-sm ring-4 ring-amber-100"
        >
          3
        </div>
      ...
      <!-- Connector -->
      <div class="h-px bg-stone-200 flex-1 -mx-8"></div>
      <!-- Step 4 -->
      <div class="flex flex-col items-center gap-2 relative z-10 flex-1 opacity-50">
        <div
          class="w-8 h-8 rounded-full bg-stone-100 border border-stone-300 text-stone-500 flex items-center justify-center font-medium text-sm"
        >
          4
        </div>
        <span class="font-metadata text-metadata text-text-secondary whitespace-nowrap">
    ...
    >
      <div
        class="p-card-padding border-b border-border-default flex justify-between items-center bg-white"
      >
        <div>
          <h2 class="font-h2 text-h2 text-text-primary">Configure Locations (Rooms)</h2>
        ...
        <div
          class="flex items-center gap-3 text-sm text-text-secondary bg-surface-subtle px-3 py-1.5 rounded-md border border-border-default"
        >
      ...
      <div class="w-full overflow-x-auto p-4">
        <DataTable :value="locations" :loading="loading" class="w-full text-left" rowHover>
          ...
          <template #footer>
            <div
              class="grid grid-cols-5 gap-4 items-center bg-amber-50/30 p-2 border border-amber-100 rounded-md mt-2"
            >
              <div>
                <select
                  v-model="newLocation.type"
                  class="w-full bg-white border border-stone-300 rounded-md py-1.5 px-3 text-sm focus:outline-none focus:border-amber-600 focus:ring-1 focus:ring-amber-600"
                >
              ...
              <div>
                <input
                  ...
                  class="w-full bg-white border border-stone-300 rounded-md py-1.5 px-3 text-sm focus:outline-none focus:border-amber-600 focus:ring-1 focus:ring-amber-600 font-mono-data"
                  placeholder="e.g. 3"
                  type="number"
                />
              </div>
